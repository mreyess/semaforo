...
int tiempo_transcurrido = 9000; // Definicion de la variable que guarda el tiempo que ha trancurrido desde que se encendio el foco verde del semaforo vehicular 
boolean boton_permiso = true;
void setup() {
...
void loop() {
 ...
 duracion = (analogRead (pot)*5)+5000;
 //Serial.println(duracion);
      ...
      while(digitalRead(boton) != LOW || tiempo_transcurrido < 10000){
        tiempo_transcurrido += 10;  
        delay(10);

        if(digitalRead(boton) == LOW && tiempo_transcurrido > 10000){
        break;
        }
